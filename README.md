...
一个功能强大的Telegram机器人，专门用于克隆和保存来自公开和私密频道的消息内容。支持流量监控、批量转发、自定义配置等功能，具备完善的错误处理和自适应速率限制机制。
...
1. **bot** (Telethon) - 主机器人客户端，用于事件处理和消息转发
...
3. 消息获取 → `pyroplug.py:get_msg()`从源频道获取消息
...
- **message_history**: message_id, chat_id, media_type, file_size, status
- **user_stats**: total_forwards, total_size per user
